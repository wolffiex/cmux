...
export function initDirPickerState(currentPath: string): DirPickerState {
  ...
  const selectedBranch =
    cousins.length > 0 ? getGitBranch(join(parentPath, cousins[0])) : null;
 ...
 */
export function getCousinDirectories(
  currentPath: string,
  windowPaths?: Set<string>,
): string[] {
  const parentPath = dirname(currentPath);
  const currentName = basename(currentPath);

  debugLog(
    `getCousinDirectories: currentPath=${currentPath}, currentName=${currentName}`,
  );
  debugLog(
    `  windowPaths provided: ${windowPaths ? JSON.stringify([...windowPaths]) : "undefined"}`,
  );
  ...
  try {
    ...
    const siblings = entries
      .filter(
        (e) =>
          e.isDirectory() && !e.name.startsWith(".") && e.name !== currentName,
      )
      .map((e) => e.name);
...
export function renderDirPicker(
  ...
  for (let i = 0; i < listHeight; i++) {
    ...
    if (itemIndex < filtered.length) {
      ...
      const maxNameLen =
        boxWidth - 4 - prefix.length - suffix.length - branchVisualLen;
      const displayName =
      ...
      const line =
        visibleLine + branchSuffix + " ".repeat(Math.max(0, padding));
...
export function renderDirPickerLines(
  ...
  for (let i = 0; i < listHeight; i++) {
    ...
    if (itemIndex < filtered.length) {
      ...
      const maxNameLen =
        boxWidth - 4 - prefix.length - suffix.length - branchSuffix.length;
      const displayName =
        name.length > maxNameLen
          ? `${name.slice(0, maxNameLen - 1)}\u2026`
          : name;
