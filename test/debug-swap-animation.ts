...
function _padToWidth(str: string, width: number): string {
...
function testAnimation() {
  ...
  const totalExpectedWidth =
    buttonBoxWidth + gap + swapZoneWidth + gap + buttonBoxWidth; // 51 chars
  ...
  for (let frame = 0; frame <= WINDOW_SWAP_FRAMES; frame++) {
    ...
    for (let row = 0; row < 4; row++) {
      ...
      const status =
        width === expectedRowWidth
          ? "OK"
          : `ERROR: expected ${expectedRowWidth}`;
  ...
  if (hasErrors) {
    console.log(
      "RESULT: FAILED - Row widths vary during animation (causes carousel churn)",
    );
    process.exit(1);
  } else {
    console.log("RESULT: PASSED - All frames have identical total width");
    console.log(
      "                 Both windows remain fully visible throughout",
    );
    console.log(
      "                 Windows visually trade positions within the swap zone",
    );
    process.exit(0);
  }
}
