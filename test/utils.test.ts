...
describe("sanitizeWindowName", () => {
  test("preserves repo/branch format under 50 chars", () => {
    ...
    expect(sanitizeWindowName("research-apps/v2-rewrite")).toBe("research-apps/v2-rewrite")
    expect(sanitizeWindowName("anthropic/claude-code-internal")).toBe("anthropic/claude-code-internal")
    expect(sanitizeWindowName("cmux/feature-branch-name")).toBe("cmux/feature-branch-name")
  })
...
describe("splitWindowName (display layer)", () => {
  ...
  test("truncates each line individually to 15 chars", () => {
    const [line1, line2] = splitWindowName("very-long-repo-name/very-long-branch-name")
